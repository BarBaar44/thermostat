...
1. **The boiler's own ESPHome PID loop** (`thermostat.yaml`'s `climate: - platform: pid`), which reads that winning room's current temperature and continuously adjusts the boiler's flow water temperature (`t_set`) to bring that specific room up to its target.
