...
It's important to be precise about what Home Assistant actually controls here, because it's *not* direct valve control. What `automations/trv_setpoint_sync.yaml` does is send each room's TRV(s) a **target temperature** (the room's virtual thermostat setpoint, +1°C) - nothing more. **I don't have any way to directly command a TRV's valve position from Home Assistant.** Every TRV listed in this repo has its own onboard temperature sensor and its own local closed-loop control logic built into the device itself: it continuously compares its own sensed temperature against the target it was last given, and opens or closes its valve accordingly, entirely on its own, without needing to hear from Home Assistant again until the target changes.
