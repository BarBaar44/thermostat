...
Every TRV also runs its own internal control loop (effectively its own small PID-like controller), which means it doesn't wait until the room is exactly at target before it starts closing - it begins throttling the valve as it *approaches* the target, to avoid overshooting.

This becomes a problem specifically for **passenger rooms** - any room that is *not* currently the Tier 1 winner driving the boiler. The boiler's flow water temperature is being held at whatever the winning room needs, which is often hotter than what a passenger room actually requires. As a passenger room's TRV senses itself nearing its own (cooler) target, it starts throttling closed to avoid overshooting - while the boiler, unaware of and unconcerned with that specific room, keeps pushing hot water through the system to satisfy the winning room. The result is two independent controllers effectively fighting each other: the boiler pushing more/hotter water in, the TRV restricting flow back out. Beyond being inefficient, this flow restriction can trip a low-flow or pressure fault on the boiler itself.

The fix is a deliberate **+1°C overshoot allowance**: each TRV is given a target 1°C above the room's actual desired temperature, rather than the exact value. This gives the TRV enough headroom that it doesn't start clamping down right at the moment the boiler is still actively serving a hotter-running winning room, removing the structural conflict between the two controllers.

Strictly speaking, this overshoot buffer is only *necessary* for passenger rooms - the Tier 1 winning room's own target already exactly matches what the boiler is producing, so there's no structural conflict for it to begin with; the boiler and that room's TRV are already moving in lockstep. In the current implementation, though, `automations/trv_setpoint_sync.yaml` applies the same +1°C to every room it syncs, regardless of whether that room happens to be the current winner - this is done for simplicity, since giving the winning room's TRV the same small buffer is harmless (it just closes marginally later, which doesn't create any new conflict).
...
Triggered whenever the `temperature` attribute of any virtual thermostat changes. Each of the six room-sync actions is individually gated by an `if:` condition checking `trigger.entity_id == <that room's virtual thermostat>` **and** that its `temperature` attribute `is not none`, before pushing the new setpoint out to that room's physical TRV(s) (with a **+1°C** overshoot buffer - see [Why each TRV target gets a +1°C overshoot buffer](#why-each-trv-target-gets-a-1c-overshoot-buffer) above for the full reasoning: it prevents the TRV's own internal control loop from throttling closed while the boiler is still actively serving a hotter-running "winning" room elsewhere in the house).
