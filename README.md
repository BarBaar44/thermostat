...
A single boiler only has **one** flow water temperature (`t_set`) - it cannot natively know or care that "the office wants 20°C while the sports room wants 16°C" at the same time. This system resolves that with two independent tiers that work together:
...
**Tier 2 - each room's TRV self-regulates its own valve:**
It's important to be precise about what Home Assistant actually controls here, because it's *not* direct valve control. What `automations/trv_setpoint_sync.yaml` does is send each room's TRV(s) a **target temperature** (the room's virtual thermostat setpoint, plus a small overshoot buffer - see below) - nothing more. **I don't have any way to directly command a TRV's valve position from Home Assistant.** Every TRV listed in this repo has its own local closed-loop control logic built into the device itself: it continuously compares a room temperature reading against the target it was last given, and opens or closes its valve accordingly, entirely on its own, without needing to hear from Home Assistant again until the target changes. As covered in [Room Temperature Sensors](#room-temperature-sensors) above, that comparison uses the same externally-pushed LYWSD03MMC reading the virtual thermostat itself uses - not the TRV's own onboard sensor - so the TRV's decision-making is working from the same accurate number as everything else in the system.

So the actual flow is: **virtual thermostat → decides room demand (drives Tier 1) and sends a target to the TRV (initiates Tier 2) → the TRV's own firmware takes it from there.** As long as the boiler is producing water hot enough to physically reach that TRV's target, the TRV will keep its valve open and let heat through. Once the room reaches its target, the TRV's own internal logic closes the valve by itself to stop the flow - Home Assistant isn't polling it every few seconds to tell it to close; the TRV decides that autonomously, the same way it would if it were paired with any other thermostat.
...
The fight this buffer prevents isn't between the boiler and *other* rooms' TRVs - it's between the boiler's **own PID loop** and the TRV **in the same room that is currently driving the boiler** (the Tier 1 winner). Two independent controllers end up watching the same room at the same time:

1. **The boiler's own ESPHome PID loop** (`thermostat.yaml`'s `climate: - platform: pid`), which reads that winning room's current temperature and continuously adjusts the boiler's flow water temperature (`t_set`) to bring that specific room up to its target. This gradual adjustment of flow temperature is the boiler **modulating**: rather than switching abruptly between fully on and fully off, the boiler continuously varies its output to approach the target smoothly.
2. **That same room's physical TRV**, running its own local control logic off the same room temperature reading, deciding independently when to start closing its own valve.

Because these are two separate, uncoordinated controllers reacting to the same room, they can easily get out of step: the boiler's PID is deliberately gentle (`kp: 0.7`, `ki: 0.005` - see [Thermostat](#thermostat) above), so it keeps modulating - producing hot flow water for a while - as it gradually approaches the target. If the TRV in that same room starts throttling its own valve closed slightly *before* the boiler has eased off its modulation, the boiler is left pushing hot water into a system with reduced flow - the classic "fight": the boiler pumping more hot water in, the TRV independently restricting it back out. Beyond being inefficient, this can trip a low-flow or pressure fault on the boiler itself.

Giving the *winning* room's TRV a target 1°C above what the boiler is aiming for gives that TRV enough headroom that it won't start closing until after the boiler's modulation has already had a chance to ease off naturally - removing the timing conflict between the two independent controllers.

### Current limitation: this +1°C buffer is applied statically to every room

Right now, `automations/trv_setpoint_sync.yaml` applies the same +1°C buffer to every room's TRV unconditionally (aside from the hallway, which is derived differently - see below), regardless of whether that room is the current Tier 1 winner or not. This is done purely for simplicity, but it has a real (if minor) side effect: **every passenger room quietly overshoots its own target by up to 1°C**, since its TRV won't fully close until it reaches target+1 - even though a passenger room's TRV was never at risk of fighting the boiler's modulation in the first place (the boiler only ever modulates to satisfy the *winning* room, not passenger rooms).

**Planned improvement (not yet implemented):** make the overshoot dynamic - only apply the +1°C buffer to whichever room `sensor.delta_temperature` currently reports as the Tier 1 winner (its `area` field), and send every other (passenger) room its exact, unmodified virtual thermostat target with no buffer at all. Since the Delta Temperature sensor already identifies the winning room every 10 seconds, this would mostly be a matter of teaching `trv_setpoint_sync.yaml` to compare each room against `sensor.delta_temperature`'s current `area` and only add the buffer when they match. This would eliminate the unnecessary overshoot on every non-winning room, while still fully protecting the winning room from the boiler-vs-TRV fight described above.
...
This firmware is what the rest of the repo (the automations, scripts, and Delta Temperature sensor) ultimately feeds - all of `climate.thermostat_central_heating`'s setpoint changes from `automations/trv_setpoint_sync.yaml` flow down into this device's PID loop, which then talks OpenTherm to the actual boiler. This is the Tier 1 half of the two-tier model described above - the PID loop's whole job is to get the boiler's actual flow water temperature to match whatever the "winning" room currently demands, by modulating output smoothly rather than switching abruptly on/off. This same modulating PID loop is also the other half of the boiler-vs-TRV fight described in [Why each TRV target gets a +1°C overshoot buffer](#why-each-trv-target-gets-a-1c-overshoot-buffer) above.

# Automations

## `automations/trv_setpoint_sync.yaml` - TRV Setpoint Sync

Triggered whenever the `temperature` attribute of any virtual thermostat changes. Each of the six room-sync actions is individually gated by an `if:` condition checking `trigger.entity_id == <that room's virtual thermostat>` **and** that its `temperature` attribute `is not none`, before pushing the new setpoint out to that room's physical TRV(s) with a **+1°C** overshoot buffer, applied statically to every room today - see [Why each TRV target gets a +1°C overshoot buffer](#why-each-trv-target-gets-a-1c-overshoot-buffer) above for the reasoning (preventing a fight between the boiler's own modulating PID loop and the TRV in whichever room is currently driving the boiler), and the [known limitation/planned improvement](#current-limitation-this-1c-buffer-is-applied-statically-to-every-room) to make this buffer apply only to the currently-winning room instead of every room uniformly.

This is the Tier 2 half of the two-tier model described in [System overview](#how-multiple-rooms-heat-at-once-the-two-tier-control-model) above: this automation only ever sends a **target temperature** to each room's TRV(s), based purely on that room's own virtual thermostat - it never tells a TRV to open or close directly. What the TRV does with that target (opening/closing its own valve to reach and hold it) is entirely up to the TRV's own onboard logic, completely independent of whichever room is currently winning the boiler's flow-temperature race.
